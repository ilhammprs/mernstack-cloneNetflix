...
export const getData=()=>{
    return async(dispatch)=>{
        const data=await fetch("http://localhost:8081/movie");
...
export const addtolist=(data,toast)=>{
    return async(dispatch)=>{
        const data2=await fetch(`http://localhost:8081/movie/addlist/${data._id}`,{
            method:"POST",
            headers:{
...
export const removeList=(id,toast)=>{
    return async(disptach)=>{
        const data2=await fetch(`http://localhost:8081/movie/list/${id}`,{
            method:"DELETE",
            headers:{
                "content-type":"application/json",
                "authorization":`bearer ${localStorage.getItem("TOKEN")}`
...
export function loginUser(data,toast){
    return async function (dispatch,getState){
          try{
            const sendData=await fetch("http://localhost:8081/user/login",{
                method:"POST",
...
export function signupUser(data,toast){
    return async function (dispatch,getState){
          try{
            const sendData=await fetch("http://localhost:8081/user/signup",{
                method:"POST",
                headers:{"content-type":"application/json"},
                body:JSON.stringify(data)
            })
...
export const removeFromList=(id)=>{
    return  async()=>{
        const deleteList=await fetch(`http://localhost:8081/movie/list/${id}`,{
            method:"DELETE",
...
export const getMyList=()=>{
    return async(dispatch)=>{
        const data=await fetch("http://localhost:8081/movie/list/all",{
            method:"GET",
